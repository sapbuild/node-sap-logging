...
describe('norman-logging performances', function () {
    var output = new logging.output.CircularBuffer({ size: 2 * testCount });
    var mockAppender = {
        level: 'info',
        output: output
    };
    ...
    it('overhead of not logging should be less than 1 µs', function () {
        ...
        assert.equal(output.length, 0, 'no message should have been logged');
    ...
    it('simple message logging should take less than 10 µs', function () {
        ...
        assert.equal(output.length, testCount, 'messages should have been logged');
